...
cpu_widget =  awful.widget.watch('bash -c "echo $[100-$(vmstat 1 2|tail -1|awk \'{print $15}\')]"', 5,
	function(widget, stdout)
		...
		widget:set_markup(string.format(w_sep .. '<span color="%s">CPU: %.0f%%</span>' .. w_sep, color, val))
	end
)
...
mem_widget =  awful.widget.watch('bash -c "free | grep Mem | awk \'{print $3/$2 * 100.0}\'"', 5,
	function(widget, stdout)
		...
		widget:set_markup(string.format('<span color="%s">MEM: %.0f%%</span>' .. w_sep, color, val))
	end
)
...
thermal_widget =  awful.widget.watch(
	...
	function(widget, stdout)
		...
		widget:set_markup(string.format('<span color="%s">TH: %.0f°C</span>' .. w_sep, color, val))
	end
)
...
capi.awesome.connect_signal("xkb::group_changed", function ()
	awful.spawn.easy_async(dbus_cmd, function(stdout, stderr, reason, exit_code)
		...
		keyboard_widget.markup = string.format('<span color="%s">%s</span>' .. w_sep, color_m, kbdd_locales[kbdd_value])
	end)
end)
